...
| **E3 rule-as-code**（v0.3 也可） | rule/skill 只是 AI prompt，不能 CI 跑 | rule frontmatter 声明 `engine + detect`，`pl-rule-scan.sh` 通用引擎消费 | ⏳ 待做 |
...
- `rules[].engine` / `rules[].spec` — E3 消费（待 v0.3）
