...
服务端实现：dashboard-server.py 直接 invoke `scripts/pl-contract-verify.sh --json`（等价于 CLI 的 `pl contract verify --json`，跑脚本是为了稳健不依赖 PATH），
...
### 静默日志 → 看详细日志
```bash
PL_DASHBOARD_VERBOSE=1 pl dashboard
```
