...
<p align="center">
  <img alt="version" src="https://img.shields.io/badge/version-v1.8.4-blue">
...
**当前稳定版：`pl-v1.8.4`（2026-04-24 · monorepo quickstart + demo-trio）**。
形态：bash 脚本 + Python 协作库，无需 npm install，macOS bash 3.2 / Linux 都能跑。
v1.8 起所有脚本通过 `pl <subcmd>` 一个入口分发。

> **多模块项目？**（前端 + Python 服务端 + 爬虫等）
> 直接看 [`docs/guides/monorepo-quickstart.md`](./docs/guides/monorepo-quickstart.md) +
> [`examples/demo-monorepo-trio/`](./examples/demo-monorepo-trio/)，下面的"单项目"流程是它的子集。

```bash
# 1. 拉本仓库并锁版本，启用 pl CLI
git clone https://github.com/walkertop/pl-pipeline-standalone
cd pl-pipeline-standalone
git checkout pl-v1.8.0
export PL_HOME="$PWD"
export PATH="$PL_HOME/bin:$PATH"     # ← 新增：让 pl 命令全局可用
...
# 3. 创建第一个变更（一条命令搞定 .state.md + 初始 trace 事件）
export PL_PROJECT="$PWD"
pl init add-user-login --name "添加用户登录" --domain auth

# 4. 跑状态 + 机器门禁（D = IMPLEMENT 验收门）
pl status add-user-login
pl run --change add-user-login --gate D --json

# 5. 冒烟（真启动应用 + HTTP probe）
pl smoke --change add-user-login --json

# 6. 合规扫描（契约漂移 + rule-as-code + CDC 反查）
pl piao contract-drift --change add-user-login --json
pl rule-scan --change add-user-login --json
pl contract verify --change add-user-login        # v1.7+ CDC 对账

# 7. 看完整合流 trace（所有通道在同一个 jsonl 里）
