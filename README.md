...
**当前稳定版：`pl-v1.12.0`（2026-04-24 · 多 IDE sync 扩展到 Cursor / CodeBuddy / Claude Code / Codex）**。
...
**v1.10.0 起：先扫描，再决定**。`pl new --here` 不带 `--stack` 默认只读 dry-run：

```bash
cd /your/existing/project

# 1) 先看 pl 怎么看你的项目（不写文件，纯建议）
pl detect                                  # 或等价: pl scan
# 输出：检测到的模块 + 每个模块的 stack 识别 + 推荐做什么

# 2) 看完建议自己拍板，再装：
pl new whatever --here --stack bare        # 只 inject pl 骨架，零侵入
pl new whatever --here --stack fastapi     # 同时装 fastapi adapter
pl new whatever --here --stack nextjs      # 同时装 nextjs adapter

# 3) 不确定？直接跑 pl new --here，自动调 detect 给提示
pl new whatever --here                     # 等价于 pl detect + 提示下一步
```

`pl detect` 识别清单（v1.10.0）：

| 检测 | 触发条件 | 给出的建议 |
|---|---|---|
| Next.js | `package.json` 有 `next` 依赖 | 装 `adapter-nextjs-web` |
| Vue + pnpm monorepo | `pnpm-workspace.yaml` + Vue 依赖 | 裸 pl-core + 自定义 build.yaml |
| FastAPI | `pyproject.toml` 含 fastapi | 装 `adapter-python-fastapi` |
| Scrapy | `pyproject.toml` 含 scrapy | 装 crawler stack |
| dlt + Prefect | `pyproject.toml` 含 dlt/prefect | 裸 pl-core + 自定义 build.yaml |
| OpenAPI 文件 | `openapi.{yaml,json}` 任意位置 | 把这里当 CDC 锚点 |
| 成熟规格文档 | `docs/` 下 ≥ 8 份 .md | 跳过 SPEC 阶段，link 现有文档 |
| 前端骨架未初始化 | `apps/` + `packages/` 但无 package.json | 提示先 pnpm init |
| Python 骨架未初始化 | `src/` + `tests/` 但无 pyproject.toml | 提示先 uv init / poetry init |
| 已接入 pl | 模块根有 `pl/config.yaml` | 不重复装；提示用 `pl init` 起新 change |
...
### 核心机器闭环（v1.2 起稳定，Agent Loop 为分支预览）

| 命令 | 作用 | 对应能力 | 产物 |
|---|---|---|---|
| `pl run --gate D` | 按 check 结果自动派生 gate 通过/阻塞 | **E1** 可执行契约层 | `trace` 事件 `gate.eval` |
| `pl agent run` | 执行 agent 命令 → gate 验证 → repair 重试 | **Agent Loop MVP** 控制平面闭环 | `agent.*` trace + repair context |
...
### Agent Execution Loop MVP（分支预览）

`pl agent run` 把外部 coding agent 的一次执行接入 PL 控制平面。它目前只内置本地 shell executor，定位是 **control plane MVP**，不是另一个 coding agent。

```bash
PL_PROJECT=/path/to/project pl agent run \
  --change add-search \
  --task T03 \
  --executor local \
...
v1.10 已经把"AI-First R&D 操作系统"的最小可用闭环交付完整：
**6 阶段 + 7 门禁 + 7 产物 + CDC 双向闭环 + 统一 CLI + 可视面板 + 一键安装 + 智能 detect**。

下一步可能方向（按 issue 排优先级，**未承诺时间**）：

- 外部 executor adapter：Codex / Claude Code / Cursor 的后台任务接入 `pl agent run`
- trace OTEL 化：给 agent/model/provider/tool/token 等维度留标准观测面
- gate repair policy：从单一 `--repair-cmd` 演进到结构化失败分类与修复策略
- 阶段语义糖：`pl proposal/plan/implement/verify/archive` 包装 `pl init + pl run --gate <X>`
- adapter 生态扩展：Go / Rust / Android / KMP-Kuikly
- retro-miner 上真实 trace（B→C→D 链路，依赖 v1.7 CDC pact 积累）
- MCP Server：让任何 MCP 兼容 IDE 直连 pl-pipeline

社区贡献欢迎在本 repo 提 issue / PR 讨论。

> **不在路线上的事**（[ROADMAP.md](./ROADMAP.md) 非目标）：
> ❌ 不重写核心为 Node.js / TypeScript / Python（坚持 bash + python3 stdlib）。
> Node.js CLI 实验存放在 [`experiments/cli-nodejs/`](./experiments/cli-nodejs/)，
> 不进 `install.sh`、不打 `pl-vX` tag。
...
- ✅ 一键 `curl | bash` 安装器 + `pl new` 项目脚手架（v1.9.0）
- ✅ `pl detect` / `pl new --here` 智能识别 + 安全 dry-run（v1.10.0）
- ✅ `pl ide sync` 支持 Cursor / CodeBuddy / Claude Code / Codex（v1.11.0 → v1.12.0）
- 🧪 `pl agent run` 最小执行闭环已在本分支 dogfood，外部 executor 接入仍未承诺
- ✅ CI 全绿，已落地"打 tag 前 CI 必须绿"纪律（v1.7.3.1 教训）
- ✅ 已有 adapter：Next.js / FastAPI / Kotlin KMM（前两个完整 CDC dogfood）
- 🚧 v1.13+ 路线见 [Issues](https://github.com/walkertop/pl-pipeline-standalone/issues)，未承诺时间
